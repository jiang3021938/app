...
class AIHubService:
    ...
    def _convert_content_item(self, item: Any) -> Dict[str, Any]:
        """Convert a single content item to a plain dict, avoiding model_dump() compatibility issues."""
        if isinstance(item, dict):
            return item
        
        # Handle ContentPartText
        if hasattr(item, 'type') and hasattr(item, 'text') and item.type == 'text':
            return {"type": "text", "text": item.text}
        
        # Handle ContentPartImage
        if hasattr(item, 'type') and hasattr(item, 'image_url') and item.type == 'image_url':
        ...
        # Fallback: try to convert using dict() or model_dump() with explicit parameters
        if hasattr(item, '__dict__'):
            return dict(item.__dict__)
        
        # Last resort: return as-is if it's already a basic type
        return item

    def _convert_message(self, msg) -> dict:
        """Convert message format and support multimodal content."""
        content = msg.content
        # If content is a list (multimodal), convert each item to plain dict
        if isinstance(content, list):
            content = [self._convert_content_item(item) for item in content]
